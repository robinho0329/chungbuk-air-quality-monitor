...
# ============================================================
# 기상청 ASOS (종관기상관측) — Phase 4 풍향·기상 분석
# ============================================================
# 5개 대기질 측정소는 모두 청주권(반경 ~15km)이라 단일 ASOS 지점(청주=131)을 공유한다.
# 지점번호/좌표 출처: 기상청 지상관측 지점정보.
KMA_ASOS_STATION_ID: int = 131  # 청주
KMA_ASOS_STATION_COORD: tuple[float, float] = (36.6392, 127.4407)

# 주요 산단 배출원 중심 좌표(근사) — 풍향 회귀에서 '산단→측정소' 방위 계산용.
INDUSTRIAL_SOURCES: dict[str, tuple[float, float]] = {
    "오창과학산업단지": (36.7220, 127.4360),   # 이차전지·반도체
    "청주산업단지": (36.6400, 127.4500),        # 전자·화학(복대·봉명 인접)
    "오송생명과학단지": (36.6300, 127.3280),    # 바이오·제약
}
